...
class PostModelTestCase(BaseTestClass):
    ...
    def test_title_slug(self):
        with self.app.app_context():
            ...
            post = Post(user_id=admin.id, title='Post de prueba', contenido='Lorem Ipsum')
            post.save()
    ...
    def test_title_slug_duplicated(self):
        with self.app.app_context():
            admin = User.get_by_email('admin@xyz.com')
            post = Post(user_id=admin.id, title='Prueba', contenido='Lorem Ipsum')
            post.save()
            post_2 = Post(user_id=admin.id, title='Prueba', contenido='Lorem Ipsum Lorem Ipsum')
            post_2.save()
            self.assertEqual('prueba-1', post_2.title_slug)
            post_3 = Post(user_id=admin.id, title='Prueba', contenido='Lorem Ipsum Lorem Ipsum')
            post_3.save()
